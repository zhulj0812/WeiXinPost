...
def get_access_token():
    ...
    print("get_access_token resp:", r.text)
    j = r.json()
...
def send_hello_world(openid: str, access_token: str):
    """
    测试号【模板消息】发送接口
    使用 config.template_id2 这个 hello world 模板
    """
    ...
    payload = {
        ...
        "url": "http://weixin.qq.com/download",
        "topcolor": "#FF0000",
        # ✅ 如果你的 hello world 模板没有任何 {{xxx.DATA}} 占位符，这里可以传空 dict
        "data": {}
    }

    resp = requests.post(url, json=payload, timeout=10)
    print(f"send to {openid} resp:", resp.text)

    j = resp.json()
    # errcode == 0 才算成功
    if j.get("errcode", -1) != 0:
        raise RuntimeError(f"template/send failed for {openid}: {j}")
...
def main():
    ...
    users = config.user
    print("users:", users)
    print("template_id2:", config.template_id2)

    for openid in users:
        send_hello_world(openid, access_token)
